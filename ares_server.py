...
def extract_and_save_facts(user_text: str, ares_text: str):
    if not GEMINI_API_KEY:
        return
    prompt = f"""Analiza esta conversación y extrae SOLO hechos claros y útiles sobre el usuario.
...
def startup():
    init_db()
    logger.info("ARES v4 iniciado correctamente")
...
.orb {
    ...
    height: 120px;
    border-radius: 50%;
    background: radial-gradient(circle at 35% 35%, #d4a5ff 0%, #9b4dff 35%, #5c1db8 70%, #2a0a5e 100%);
    box-shadow: 0 0 40px rgba(155, 77, 255, 0.55), 0 0 80px rgba(120, 40, 220, 0.3), inset 0 0 30px rgba(255,255,255,0.12);
    position: relative;
    z-index: 10;
}
.ring {
...
const WAKE_PHRASES = ["hey ares", "oye ares", "ok ares", "okay ares", "hola ares", "ares", "ei ares", "ey ares"];
...
function initRecognition() {
    ...
    recognition.onresult = function(event) {
        ...
        if (mode === "wake") {
            const detected = WAKE_PHRASES.some(function(p) { return transcript.includes(p); });
            if (detected) {
                onWakeWord();
            }
...
async function processCommand(texto) {
    ...
    try {
        ...
        if (contentType.includes("audio")) {
            ...
            isProcessing = false;
            setTimeout(function() { startWakeListening(); }, 4000);
        }
    } catch (err) {
        console.error(err);
        statusEl.textContent = "Error de conexión con el servidor";
        isProcessing = false;
        setTimeout(function() { startWakeListening(); }, 3000);
    }
}
...
async def preguntar(prompt: str = Form(...)):
    if not prompt.strip():
        raise HTTPException(status_code=400, detail="Vacío")
